...
export function Dashboard({ onViewChange, onCreateAgent }: DashboardProps) {
  ...
  const fetchData = async (isRefresh = false) => {
    ...
    try {
      const [
        ...
        chatApi.getSessionsByUserId(currentUserId).catch(() => []),
      ]);

      const agents = agentsData.status === 'fulfilled' && Array.isArray(agentsData.value) ? agentsData.value : [];
      const knowledge = knowledgeData.status === 'fulfilled' && Array.isArray(knowledgeData.value) ? knowledgeData.value : [];
      const sessions = sessionsData.status === 'fulfilled' && Array.isArray(sessionsData.value) ? sessionsData.value : [];
      const messageGroups = await Promise.all(sessions.map(session => chatApi.getMessagesBySessionId(session.ulid).catch(() => [])));
      const messages = messageGroups.flat() as ChatMessage[];
      const sessionIds = new Set(sessions.map(session => session.ulid));
      const agentNames = new Map(agents.map(agent => [agent.ulid || agent.id, agent.name]));

      const completedTasks = messages.filter(message => message.role === 'assistant' && message.status === 'completed').length;
      const totalTokens = messages.reduce((sum, message) => sum + (Number(message.tokens) || 0), 0);
      setOverview({
        active_agents: agents.filter(agent => agent.enabled !== false).length,
        periodic_agents: agents.filter(agent => agent.enabled !== false && (agent.is_periodic || agent.isPeriodic || agent.cron_rule || agent.cronRule)).length,
        tasks_completed: completedTasks,
        total_tokens: totalTokens,
        active_knowledge_sources: knowledge.filter(source => source.enabled !== false).length,
      });

      const sessionById = new Map(sessions.map(session => [session.ulid, session]));
      const tokensByAgent = new Map<string, number>();
      messages.forEach(message => {
        const agentId = sessionById.get(message.session_id)?.agent_id;
        if (!agentId) return;
        tokensByAgent.set(agentId, (tokensByAgent.get(agentId) || 0) + (Number(message.tokens) || 0));
      });
      setTokenRanking([...tokensByAgent.entries()]
        .map(([agent_id, total_tokens]) => ({ agent_id, agent_name: agentNames.get(agent_id) || '未知 Agent', total_tokens }))
        .sort((a, b) => b.total_tokens - a.total_tokens));

      const channels = new Map<string, number>();
      sessions.forEach((session, index) => {
        const channel = session.channel || 'web';
        const messageCount = messageGroups[index]?.length || 0;
        channels.set(channel, (channels.get(channel) || 0) + messageCount);
      });
      setChannelActivity([...channels.entries()].map(([channel, message_count]) => ({
        channel_id: channel,
        channel_name: channel === 'web' ? '网页聊天' : channel,
        status: 'active' as const,
        message_count,
      })));

      const approvals = approvalsData.status === 'fulfilled' && Array.isArray(approvalsData.value) ? approvalsData.value : [];
      setPendingApprovals(approvals.filter(approval => sessionIds.has(approval.session_id)));
  ...
  const stats = [
    ...
    },
    {
      ...
      value: overview ? formatNumber(overview.tasks_completed) : '-',
      change: '',
      trend: 'up',
      icon: Zap,
