...
export default function ResearchSourcesPanel({ pages, queryTexts = [], confidence = 0 }: Props) {
      ...
      <div className="mt-3 grid auto-cols-[minmax(245px,78%)] grid-flow-col gap-2.5 overflow-x-auto pb-2 sm:auto-cols-[minmax(270px,48%)]">
        {pages.map(page => (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="group flex min-h-44 flex-col rounded-2xl border border-slate-200/90 bg-white p-3 shadow-sm transition-all hover:-translate-y-0.5 hover:border-brand-300 hover:shadow-md"
          >
            <div className="flex items-start gap-2.5">
              <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-slate-900 text-[11px] font-black text-white">
              ...
              <ExternalLink size={13} className="shrink-0 text-slate-300 transition-colors group-hover:text-brand-500" />
            </div>
