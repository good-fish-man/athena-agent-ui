...
type Intent = 'add_model' | 'create_agent' | 'install_skill' | 'show_inbox' | 'config_kb' | 'test_kb_recall' | 'open_workspace' | 'open_settings' | 'open_models' | 'open_agents' | 'open_skills' | 'open_chat' | 'agent_result' | 'unknown';
...
export function CommandCenter({ onViewChange }: CommandCenterProps) {
  ...
  const handleCommand = async () => {
    ...
    try {
      ...
      // Prefilled management actions require confirmation; Agent results have
      // already been executed by the backend and should be rendered directly.
      const needsConfirmation = !!cmdResult.prefilled;
      const alreadyExecuted = cmdResult.action === 'agent_result';
      ...
      const newAction: CommandAction = {
        ...
        status: needsConfirmation ? 'pending' : alreadyExecuted ? 'completed' : 'executing',
        result: cmdResult
      };
      ...
      // 无需确认的直接执行
      if (!needsConfirmation && !alreadyExecuted) {
      ...
      toast.error(error instanceof Error ? error.message : t('commandCenter.failed'));
    } finally {
  ...
  const getIcon = (intent: Intent) => {
    switch (intent) {
      ...
      case 'agent_result': return <Bot className="text-brand-500" size={18} />;
      case 'open_workspace': return <Zap className="text-cyan-600" size={18} />;
