...
- Experience 工作区支持脱敏任务历史、保留策略、失败分析和确定性离线回归套件。
- Release Control 内置签名 Provider Registry，可完成不可变安装、权限评审、公共发布门禁、撤销/重载和调用审计。
...
打开 **Experience** 可以检查经过脱敏的历史任务结果，按状态、结果和敏感级别检索，调整用户级学习/保留设置，或永久删除某条经验的内容。用户可以把选中的经验冻结成 Fixture，并通过确定性离线 Suite 重放。历史经验始终被标记为不可信、只读证据，不会覆盖当前环境的最新 Observation。
